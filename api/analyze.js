    ...
    attributes: {
      ...
      items: {
        ...
        properties: {
          ...
          label: { type: "STRING", description: "日本語の短い体言止めラベル（例: 無添加、一番摘み、静音性）。「〜を重視する」「〜を好む」のような丁寧な言い回しの語尾は付けず、名詞・名詞句だけで簡潔に表す" },
        },
        required: ["id", "label"],
      },
    },
...
1. attributes: 「${keyword}」というジャンルの買い物で、価格以外にユーザーが気にしそうな「好み・こだわり」の観点を2〜4個選び、id（英数字、アンダースコア可）とlabel（日本語、名詞・名詞句だけの短い体言止め。例:「無添加」「一番摘み」「静音性」「フェアトレード」）で挙げてください。「〜を重視する」「〜を好む」のような丁寧な語尾は付けず、チェックボックスの横にそのまま表示できる短い言葉にしてください。ジャンルに応じて柔軟に選んでください（例: 食品なら無添加・産地、コーヒーならオーガニック・フェアトレード・カフェインの有無、キーボードなら無線接続・静音性・コンパクトさなど）。idに"cost"は使わないでください（価格は別枠で扱います）。
