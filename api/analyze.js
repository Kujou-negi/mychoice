...
const RESPONSE_SCHEMA = {
  ...
  properties: {
    ...
    specFields: {
      type: "ARRAY",
      description: "このジャンルの商品を比較するときに見たい、客観的な仕様・スペック項目を2〜3個。好み判断ではなく事実情報（例: 食品なら『容量』『原材料』『容器』、キーボードなら『接続方式』『キースイッチ』『サイズ』、衣類なら『サイズ』『素材』『カラー』、家電なら『対応OS』『画面サイズ』など）。ジャンルに完全に合わせて自由に選んでよい",
      items: {
        type: "OBJECT",
        properties: {
    ...
    items: {
      ...
      items: {
        ...
        properties: {
          ...
          origin: { type: "STRING", description: "産地となる都道府県名（例: 長野県）。商品名・説明文から読み取れる場合のみ。地名（紀州、南高梅など）は分かる範囲で都道府県名に変換する。読み取れない、または食品以外で産地の概念がない場合は空文字" },
...
2. specFields: 「${keyword}」というジャンルの商品を並べて比較するときに見たい、客観的な仕様・スペック項目を2〜3個選んでください。好みではなく事実情報です。ジャンルに完全に合わせて自由に選んでください（例: 食品・飲料なら『容量』『原材料』『容器』、キーボードなら『接続方式』『キースイッチ』『サイズ』、衣類なら『サイズ』『素材』、家電なら『画面サイズ』『対応OS』など）。idは英数字、labelは日本語で2〜5文字程度の短い見出しにしてください。食品ではないジャンルに『原材料』『容量(ml/g)』のような食品向け項目を無理に当てはめないでください。
...
- origin: 産地となる都道府県名を商品名・説明文から抽出してください（例:「長野県」）。地名だけが書かれている場合（紀州→和歌山県、南高梅→和歌山県など）は分かる範囲で都道府県名に変換してください。はっきり分からない場合や、食品以外で産地の概念がないジャンルの場合は空文字にしてください。
